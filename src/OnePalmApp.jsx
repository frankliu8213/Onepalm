import React, { useState, useEffect } from 'react';
import { Solar, Lunar } from 'lunar-javascript';
window.Solar = Solar;
window.Lunar = Lunar;
import { Book, Moon, Sun, RotateCcw, Info, Hand, Compass, Heart, Briefcase, AlertCircle, Sparkles, Star, GitMerge, MapPin, Scroll, User, Clock, Gem, Calendar, Users, Flower, CalendarCheck, ThumbsUp, Filter, Bot, MessageSquare, X, Printer, UserPlus, ArrowRightLeft, ShieldCheck, ShieldAlert } from 'lucide-react';

/**
 * 达摩一掌经 (Dharma One Palm Sutra) Calculator
 * Combined Version with Relationship Module
 * Features:
 * - Core Destiny Calculation
 * - Birth Guide
 * - Relationship/Compatibility Analysis (New!)
 * - Print & AI Integration
 */

// --- Constants & Data ---

const REALMS = {
  BUDDHA: { name: '佛道', color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200', icon: '卍', trait: '慈悲与福报' },
  IMMORTAL: { name: '仙道', color: 'text-sky-700', bg: 'bg-sky-50', border: 'border-sky-200', icon: '☁', trait: '逍遥与智慧' },
  HUMAN: { name: '人道', color: 'text-rose-700', bg: 'bg-rose-50', border: 'border-rose-200', icon: '웃', trait: '入世与权责' },
  ASURA: { name: '修罗', color: 'text-purple-700', bg: 'bg-purple-50', border: 'border-purple-200', icon: '⚔', trait: '好胜与智谋' },
  GHOST: { name: '鬼道', color: 'text-slate-600', bg: 'bg-slate-100', border: 'border-slate-300', icon: '👻', trait: '匮乏与奔波' },
  ANIMAL: { name: '畜道', color: 'text-orange-700', bg: 'bg-orange-50', border: 'border-orange-200', icon: '🐾', trait: '本能与劳碌' },
  EMPTY: { name: '', color: '', bg: '', border: '', icon: '', trait: '' }
};

const STARS = {
  1: {
    id: 1, branch: '子', name: '天贵星', realm: REALMS.BUDDHA,
    desc: '佛道天贵星', poem: '时辰落在天贵星，一生清贵事和同，志气不凡人出类，安然自在性明通。',
    archetype: '清高的智者',
    keywords: '智慧、清高、孤傲、慈悲',
    general_analysis: '天贵星代表精神层面的贵气。您可能对物质生活看得很淡，但对精神自由要求很高。',
    gender_specific: {
      male: '男命天贵：古诀云“两重天贵者贵而不贵”。若多见，反主虚名无实。',
      female: '女命天贵：古诀云“女命天贵重者，终许见贵”。主气质高雅，晚运安详。'
    },
    advice: '适合从事学术、哲学或宗教研究。需警惕过于清高而导致的人际孤立。',
    pos_meaning: {
      year: '年上天贵：主祖上阴德深厚，少年时期如鱼得水，学业优秀。',
      month: '月上天贵：青年时期贵人运极佳，事业起步顺风顺水。',
      day: '日上天贵：中年运势登顶，容易获得社会地位。配偶贤良。',
      hour: '时上天贵：晚年德高望重，子女成才且孝顺。'
    }
  },
  2: {
    id: 2, branch: '丑', name: '天厄星', realm: REALMS.GHOST,
    desc: '鬼道天厄星', poem: '时在厄中人混沌，惺惺作事又痴呆，此人带疾方延寿，还须劳碌作生涯。',
    archetype: '负重的行者',
    keywords: '磨难、深邃、焦虑、暗疾',
    general_analysis: '天厄并非单纯的灾难，更多是一种“阻滞”感。思想深邃，但容易怀才不遇。',
    gender_specific: {
      male: '男命天厄：主要体现为事业上的阻滞，或者身体上有慢性疾病。',
      female: '女命天厄：古诀云“女怕破刃厄”。需防妇科疾厄，主操心劳碌。'
    },
    advice: '适合从事医疗、心理咨询，化解自身业力。多布施可减轻匮乏感。',
    pos_meaning: {
      year: '年上天厄：祖业根基较弱，早年生活可能充满挑战。',
      month: '月上天厄：青年时期事业发展容易碰壁，只能靠自己打拼。',
      day: '日上天厄：中年时期需特别注意身体健康，内心深处常感孤独。',
      hour: '时上天厄：晚年需防旧疾复发，宜早做健康和养老规划。'
    }
  },
  3: {
    id: 3, branch: '寅', name: '天权星', realm: REALMS.HUMAN,
    desc: '人道天权星', poem: '时辰落在天权星，性格操持志气雄，作事差迟人也喜，一呼百喏有威风。',
    archetype: '掌权的领袖',
    keywords: '权力、责任、管理、霸道',
    general_analysis: '天权星代表了极强的领导气场。性格稳重，办事干练，天生喜欢掌控局势。',
    gender_specific: {
      male: '男命天权：主事业心强，有领导欲，必为单位一把手或创业老板。',
      female: '女命天权：古诀云“有天权星者助夫旺子”。能力极强，能操持家业。'
    },
    advice: '切忌刚愎自用。权力是责任的另一种形式，学会放权。',
    pos_meaning: {
      year: '年上天权：出身于有威望的家庭。从小显露孩子王气质。',
      month: '月上天权：青年时期在职场崭露头角，执行力强，容易成为团队核心。',
      day: '日上天权：中年掌握实权，事业达到高峰。在家庭中也是主导者。',
      hour: '时上天权：晚年依然保持威严，受人敬畏。子女有出息。'
    }
  },
  4: {
    id: 4, branch: '卯', name: '天破星', realm: REALMS.ANIMAL,
    desc: '畜道天破星', poem: '时辰落在天破宫，堆金积玉也成空，夜眠算计图家富，钞袋谁知有蛀虫。',
    archetype: '慷慨的散财童子',
    keywords: '破财、随性、善良、无计划',
    general_analysis: '天破星主财来财去。指对金钱缺乏严谨的规划，或者为人过于慷慨大方。',
    gender_specific: {
      male: '男命天破：主仗义疏财，人缘好，适合从事流动性大的行业。',
      female: '女命天破：古诀云“女怕破刃厄”。需防因感情或家庭原因破财。'
    },
    advice: '建议建立强制储蓄机制。适合慈善公关行业。',
    pos_meaning: {
      year: '年上天破：祖业难守，早年家境可能经历波折。',
      month: '月上天破：青年时期工作变动多，钱财来得快去得也快。',
      day: '日上天破：中年需防投资失利。配偶可能也不善理财。',
      hour: '时上天破：晚年钱财容易耗散，或者在子女身上花费巨大。'
    }
  },
  5: {
    id: 5, branch: '辰', name: '天奸星', realm: REALMS.ASURA,
    desc: '修罗天奸星', poem: '大如沧海细如毛，佛口蛇心两面刀，奸狡狠谋藏毒性，意多翻覆最难调。',
    archetype: '机智的谋士',
    keywords: '聪慧、谋略、好斗、机变',
    general_analysis: '天奸星代表极致的智力与策略。非常聪明，善于察言观色和利用规则。',
    gender_specific: {
      male: '男命天奸：主足智多谋，善于权术。若走正道是极佳的谋士。',
      female: '女命天奸：主心思细腻深沉，极其聪明，能把控复杂关系。'
    },
    advice: '适合律师、投资、战略顾问等。切记真诚待人。',
    pos_meaning: {
      year: '年上天奸：家族关系复杂，或幼年环境充满竞争。心智早熟。',
      month: '月上天奸：青年时期足智多谋，善于利用规则。在竞争中能脱颖而出。',
      day: '日上天奸：中年城府颇深，事业上善用手段。婚姻中可能缺乏坦诚。',
      hour: '时上天奸：晚年思维敏捷，不甘寂寞，喜欢出谋划策。'
    }
  },
  6: {
    id: 6, branch: '巳', name: '天文星', realm: REALMS.IMMORTAL,
    desc: '仙道天文星', poem: '命遇天文秀气清，聪明智慧意惺惺，男才女秀身清吉，满腹文章锦绣成。',
    archetype: '儒雅的书生',
    keywords: '文学、名声、清秀、多情',
    general_analysis: '天文星主文采风流。外表清秀，对美学、文学有天然的敏感度。',
    gender_specific: {
      male: '男命天文：主“男才”，文质彬彬，有书卷气，适合走学术路线。',
      female: '女命天文：主“女秀”，外貌清秀，心灵手巧，有文艺特长。'
    },
    advice: '务必从事创造性或文化传播类工作。注意处理感情问题。',
    pos_meaning: {
      year: '年上天文：出身书香门第。早年展现出极高的学习天赋。',
      month: '月上天文：青年时期才华横溢，文笔极佳，容易获得名声。',
      day: '日上天文：中年儒雅风流，从事文教、创意工作大吉。',
      hour: '时上天文：晚年精神生活丰富，著书立说或寄情山水。'
    }
  },
  7: {
    id: 7, branch: '午', name: '天福星', realm: REALMS.BUDDHA,
    desc: '佛道天福星', poem: '命逢天福是生时，定然仓库有盈余，宽洪大量根基稳，财帛光华百福齐。',
    archetype: '入世的贵人',
    keywords: '福报、富足、布施、亨通',
    general_analysis: '天福星代表世俗的荣华富贵。乐于享受物质生活，并且通常运气很好。',
    gender_specific: {
      male: '男命天福：主事业亨通，财官双美，是家里的顶梁柱。',
      female: '女命天福：古诀云“天福重者衣禄自然”。主旺夫益子，一生少操劳。'
    },
    advice: '福报深厚是优势，但需警惕“富贵病”。持续布施。',
    pos_meaning: {
      year: '年上天福：含着金汤匙出生，或家境殷实。从小衣食无忧。',
      month: '月上天福：青年运势顺遂，贵人多助，做什么都容易成功。',
      day: '日上天福：中年富贵双全，掌握重要资源。家庭和睦。',
      hour: '时上天福：晚景荣华富贵，子孙满堂且孝顺。'
    }
  },
  8: {
    id: 8, branch: '未', name: '天驿星', realm: REALMS.GHOST,
    desc: '鬼道天驿星', poem: '人道若逢天驿星，搬移离祖不曾停，身心不得片时静，走遍天涯是未宁。',
    archetype: '奔波的游子',
    keywords: '变动、劳碌、远行、社交',
    general_analysis: '天驿星主变动。内心深处有一种不安定感，很难在一个地方长久停留。',
    gender_specific: {
      male: '男命天驿：古诀云“男怕孤驿凶”。主终身奔波，难以安家。',
      female: '女命天驿：古诀云“二驿星反主女贵”。主见多识广，能在动中求财。'
    },
    advice: '适合外贸、旅游、物流等工作。不要强求安稳。',
    pos_meaning: {
      year: '年上天驿：祖辈可能漂泊不定，或者早年随父母频繁搬家。',
      month: '月上天驿：青年时期注定离家发展，在外求学或工作。',
      day: '日上天驿：中年奔波劳碌，常为事业四处奔走。',
      hour: '时上天驿：晚年依然闲不下来，喜欢旅游或移居海外。'
    }
  },
  9: {
    id: 9, branch: '申', name: '天孤星', realm: REALMS.HUMAN,
    desc: '人道天孤星', poem: '时辰若逢此天孤，六亲兄弟有如无，空作空门清静客，总有妻儿情分疏。',
    archetype: '孤独的修行者',
    keywords: '独立、缘薄、宗教、界限感',
    general_analysis: '天孤星主情感疏离。从小就觉得与家族格格不入，有一种强烈的“界限感”。',
    gender_specific: {
      male: '男命天孤：古诀云“男人得之六亲无分”。主性格孤僻，适合搞技术或修行。',
      female: '女命天孤：古诀云“女人得之克子妨夫”。要注意经营婚姻和亲子关系。'
    },
    advice: '适合独立性强的工作。离祖则吉，去外地发展可化解刑克。',
    pos_meaning: {
      year: '年上天孤：早年与父母缘分较薄，或者性格内向孤僻。',
      month: '月上天孤：兄弟姐妹少或关系不亲密。青年时期习惯独来独往。',
      day: '日上天孤：中年夫妻缘分淡薄，倾向于精神层面的追求。',
      hour: '时上天孤：晚景可能比较清冷，适合修行或独立研究。'
    }
  },
  10: {
    id: 10, branch: '酉', name: '天刃星', realm: REALMS.ANIMAL,
    desc: '畜道天刃星', poem: '天刃为人性大刚，是非终日要争强，持刀弄斧刑心重，好似将军入战场。',
    archetype: '刚烈的战将',
    keywords: '刚强、争斗、技术、直爽',
    general_analysis: '天刃星代表杀伐决断。性格直爽，脾气火爆，做事雷厉风行。',
    gender_specific: {
      male: '男命天刃：主性格刚毅，适合从军或做外科医生。',
      female: '女命天刃：古诀云“女怕破刃厄”。性格太刚强容易吃亏。'
    },
    advice: '适合军警、外科、机械行业。修身养性，遇事三思。',
    pos_meaning: {
      year: '年上天刃：祖上可能从事武职，或者早年性格顽劣叛逆。',
      month: '月上天刃：青年时期血气方刚，讲义气但容易冲动惹事。',
      day: '日上天刃：中年脾气火爆，夫妻常有争执。身体易有手术或外伤。',
      hour: '时上天刃：晚年性格固执，不易相处。需防意外伤害。'
    }
  },
  11: {
    id: 11, branch: '戌', name: '天艺星', realm: REALMS.ASURA,
    desc: '修罗天艺星', poem: '天艺生人性最灵，将南作北逞多能，讳为见灵机关巧，到处和同作事勤。',
    archetype: '执着的工匠',
    keywords: '才艺、技术、清高、内向',
    general_analysis: '天艺星侧重于“技艺”天赋。在某一方面（如艺术、编程）可能有惊人的天赋。',
    gender_specific: {
      male: '男命天艺：主技艺超群，能靠手艺吃饭，但可能不善言辞。',
      female: '女命天艺：古诀云“有天文天艺星者性巧”。主心灵手巧，善女红或设计。'
    },
    advice: '拥有一技之长是立身之本。不要强迫自己长袖善舞。',
    pos_meaning: {
      year: '年上天艺：祖上可能是手艺人。早年就展现出艺术天赋。',
      month: '月上天艺：青年时期学艺有成，多才多艺。但可能比较清高。',
      day: '日上天艺：中年靠一技之长立足社会，在专业领域具有权威性。',
      hour: '时上天艺：晚年生活丰富多彩，有艺术爱好相伴。'
    }
  },
  12: {
    id: 12, branch: '亥', name: '天寿星', realm: REALMS.IMMORTAL,
    desc: '仙道天寿星', poem: '夫妻生时命最长，上恭下敬性温良，一闻千悟心慈善，喜怒中间有主张。',
    archetype: '长寿的隐士',
    keywords: '长寿、温和、慢节奏、大器晚成',
    general_analysis: '天寿星主“慢”与“静”。心态极好，不争不抢，因此往往高寿。',
    gender_specific: {
      male: '男命天寿：主性格宽厚，不与人争，一生平稳，是典型的“老好人”。',
      female: '女命天寿：主慈悲善良，相夫教子，晚年尤其安乐。'
    },
    advice: '虽然有福，但不可过于懒散。依靠自己的努力能获得更踏实的成就。',
    pos_meaning: {
      year: '年上天寿：家族有长寿基因，长辈慈祥。早年生活平稳。',
      month: '月上天寿：青年时期心态平和，不争不抢。发展速度虽慢但稳健。',
      day: '日上天寿：中年身体健康，心态极好。日子过得舒心惬意。',
      hour: '时上天寿：高寿之命，晚年安乐祥和，智慧通达。'
    }
  },
};

const DAY_STARS = {
  1: '太阳星', 7: '太阳星', 13: '太阳星', 19: '太阳星', 25: '太阳星',
  2: '太阴星', 8: '太阴星', 14: '太阴星', 20: '太阴星', 26: '太阴星',
  3: '天武星', 9: '天武星', 15: '天武星', 21: '天武星', 27: '天武星',
  4: '天福星', 10: '天福星', 16: '天福星', 22: '天福星', 28: '天福星',
  5: '天皇星', 11: '天皇星', 17: '天皇星', 23: '天皇星', 29: '天皇星',
  6: '太乙星', 12: '太乙星', 18: '太乙星', 24: '太乙星', 30: '太乙星',
};

const DAY_STAR_MEANING = {
  '太阳星': '衣禄有余，初限平平，末限大好，男享荣华，女旺夫发福。',
  '太阴星': '为人秀气，父母有克，六亲无倚，男主清奇，女主聪明。',
  '天武星': '夫妻和合，子息刑伤，末限荣华，离祖成家之命。',
  '天福星': '多学少成，中限无财，惹是招非，末限富贵大吉。',
  '天皇星': '为人伶俐，衣禄有余，兄弟无倚，财利无亏中平之命。',
  '太乙星': '多学多能，清闲高贵，初限大贵，末限大旺财禄。'
};

const TIME_RANGES = {
  1: '23:00 (前一日) - 01:00', 2: '01:00 - 03:00', 3: '03:00 - 05:00',
  4: '05:00 - 07:00', 5: '07:00 - 09:00', 6: '09:00 - 11:00',
  7: '11:00 - 13:00', 8: '13:00 - 15:00', 9: '15:00 - 17:00',
  10: '17:00 - 19:00', 11: '19:00 - 21:00', 12: '21:00 - 23:00'
};

const ZODIAC_HARM = {
  1: { target: 8, desc: '子未相害（羊鼠相逢一旦休）' },
  2: { target: 7, desc: '丑午相害（从来白马怕青牛）' },
  3: { target: 6, desc: '寅巳相害（蛇逢猛虎如刀截）' },
  4: { target: 5, desc: '卯辰相害（玉兔见龙云里去）' },
  5: { target: 4, desc: '辰卯相害（玉兔见龙云里去）' },
  6: { target: 3, desc: '巳寅相害（蛇逢猛虎如刀截）' },
  7: { target: 2, desc: '午丑相害（从来白马怕青牛）' },
  8: { target: 1, desc: '未子相害（羊鼠相逢一旦休）' },
  9: { target: 12, desc: '申亥相害（猪见猿猴似箭投）' },
  10: { target: 11, desc: '酉戌相害（金鸡遇犬泪双流）' },
  11: { target: 10, desc: '戌酉相害（金鸡遇犬泪双流）' },
  12: { target: 9, desc: '亥申相害（猪见猿猴似箭投）' },
};

// --- Helper Functions ---

const getTimeSubdivision = (minutes) => {
  if (minutes < 40) return { label: '时初', desc: '初生人，先克母。作事七进八退，十成九败，六亲疏淡，末限享福。' };
  if (minutes < 80) return { label: '时中', desc: '中生人，无克破。一生作事占强，有起有倒，末后大兴旺，离祖入赘为吉。' };
  return { label: '时末', desc: '末生人，先克父。六亲不得力，一生勤苦劳碌，作事有头无尾，晚景好。' };
};

const getShi = (yearIdx, month) => {
  const yearStar = STARS[yearIdx];
  const monthStar = STARS[(yearIdx + month - 1) % 12 || 12];
  if (yearStar.realm.name === '佛道' || monthStar.realm.name === '佛道') return { name: '佛法识', desc: '生来八字分明，好学古今，心性巧慧，与佛有缘。' };
  if (yearStar.realm.name === '修罗' || monthStar.realm.name === '修罗') return { name: '方丈识', desc: '为人权柄威严，不犯刑冲多主贵，有大丈夫气概。' };
  if (yearStar.realm.name === '人道' || monthStar.realm.name === '人道') return { name: '通天识', desc: '一生衣禄坚牢，气象人尊多近贵，寿元颇高。' };
  return { name: '善知识', desc: '众人钦敬，衣禄自然丰足，到处有贵人相助。' };
};

const getTimeBranchIndex = (hour) => {
  if (hour >= 23 || hour < 1) return 1;
  if (hour < 3) return 2;
  if (hour < 5) return 3;
  if (hour < 7) return 4;
  if (hour < 9) return 5;
  if (hour < 11) return 6;
  if (hour < 13) return 7;
  if (hour < 15) return 8;
  if (hour < 17) return 9;
  if (hour < 19) return 10;
  if (hour < 21) return 11;
  return 12;
};

const calculatePosition = (startPos, steps, isClockwise) => {
  let current = startPos;
  const moves = steps - 1;
  if (isClockwise) {
    current = (current + moves) % 12;
    if (current === 0) current = 12;
  } else {
    current = (current - moves) % 12;
    if (current <= 0) current += 12;
  }
  return current;
};

const getFanZhongAnalysis = (starsCount) => {
  const findings = [];
  if (starsCount[1] === 2) findings.push({ star: '天贵', count: 2, desc: '男命两重天贵者贵而不贵，看似体面实则虚名；女命重贵者终许见贵，晚运较好。' });
  if (starsCount[2] === 2) findings.push({ star: '天厄', count: 2, desc: '厄星重犯，反不为厄，或有小疾但无大碍。' });
  if (starsCount[2] >= 3) findings.push({ star: '天厄', count: 3, desc: '古诀云“逢三厄者不唯无厄，而衣禄有余”，主因祸得福，或从事医疗/慈善业而发家。' });
  if (starsCount[3] === 2) findings.push({ star: '天权', count: 2, desc: '两重天权，为人庄重正大，不怒自威。' });
  if (starsCount[3] >= 3) findings.push({ star: '天权', count: 3, desc: '三权在命，必主威权，适合从事管理、军警或掌握实权的岗位。' });
  if (starsCount[4] === 2) findings.push({ star: '天破', count: 2, desc: '双破入命，衣禄反稳，财去财来终有积蓄。' });
  if (starsCount[4] >= 3) findings.push({ star: '天破', count: 3, desc: '三破攒聚，恐有破败之虞，需严守钱财，不可投机。' });
  if (starsCount[5] === 2) findings.push({ star: '天奸', count: 2, desc: '奸星重见，反不奸而正大，为人机智但守底线。' });
  if (starsCount[5] >= 3) findings.push({ star: '天奸', count: 3, desc: '三奸入命，恐流于狡狯，需修心养性，防因小聪明误事。' });
  if (starsCount[6] >= 2) findings.push({ star: '天文', count: 2, desc: '文星重犯，主男女情感丰富，易犯桃花，需防情感泛滥。' });
  if (starsCount[6] >= 3) findings.push({ star: '天文', count: 3, desc: '三文入命，衣禄虽少但文采斐然，适合纯粹的学者或艺术家。' });
  if (starsCount[7] === 2) findings.push({ star: '天福', count: 2, desc: '二福星者，恐克妻（或夫）而先贵，事业先成家业后立。' });
  if (starsCount[7] >= 3) findings.push({ star: '天福', count: 3, desc: '三福星者，有寿而无福（或福报太重反生懒惰），宜多布施。' });
  if (starsCount[8] === 2) findings.push({ star: '天驿', count: 2, desc: '二驿星反主贵（尤利女命），动中求财大吉。' });
  if (starsCount[8] >= 3) findings.push({ star: '天驿', count: 3, desc: '三驿星者主劳碌，一生奔波不停，难安一处。' });
  if (starsCount[9] === 2) findings.push({ star: '天孤', count: 2, desc: '二孤星者，反有子孙，并不全孤，但内心仍有疏离感。' });
  if (starsCount[9] >= 3) findings.push({ star: '天孤', count: 3, desc: '三孤入命，刑克六亲，宜晚婚或皈依宗教/从事独立研究。' });
  if (starsCount[10] === 2) findings.push({ star: '天刃', count: 2, desc: '二刃星者主慈善（物极必反），外刚内柔。' });
  if (starsCount[10] >= 3) findings.push({ star: '天刃', count: 3, desc: '三刃星者，乃贵人有权之命，但也主性情刚烈。' });
  if (starsCount[11] === 2) findings.push({ star: '天艺', count: 2, desc: '二艺星者，多才多艺但也容易刑伤手足或伴侣。' });
  if (starsCount[11] >= 3) findings.push({ star: '天艺', count: 3, desc: '三艺星者，恐蒙瞳昏愚，或艺多不压身但难精一门。' });
  if (starsCount[12] === 2) findings.push({ star: '天寿', count: 2, desc: '二寿星者性愚鲁（大智若愚），为人厚道。' });
  if (starsCount[12] >= 3) findings.push({ star: '天寿', count: 3, desc: '三寿星者，出家而高寿，或在世俗中长寿但清贫。' });
  return findings;
};

const getInteractionAnalysis = (pillars) => {
  const starIds = pillars.map(p => p.data.id);
  const has = (id) => starIds.includes(id);
  const interactions = [];

  if (has(1) && has(3)) interactions.push({ title: '贵权相佐', type: 'good', desc: '天贵星（1）逢 天权星（3）：书云“荣昌富贵”。权力的执行力配合贵人的格局，主事业有成，位高权重。' });
  if (has(1) && has(7)) interactions.push({ title: '福贵双全', type: 'good', desc: '天贵星（1）逢 天福星（7）：书云“显荣之命”。清高的名声加上世俗的福报，主一生衣食无忧，受人尊敬。' });
  if (has(3) && has(7) && has(12)) interactions.push({ title: '三吉拱照', type: 'good', desc: '权（3）、福（7）、寿（12）三星同在：书云“人人钦敬”。主宽宏大量，福寿绵绵，是难得的好格局。' });
  if (has(6) && has(1) && has(7) && has(11)) interactions.push({ title: '鳌头独占', type: 'good', desc: '文、贵、福、艺四星聚首：书云“鳌头独占”。主学识过人，金阶玉陛之人，考运极佳。' });
  if (has(6) && (has(3) || has(10))) interactions.push({ title: '文武双全', type: 'good', desc: '天文星（6）逢 天权（3）或 天刃（10）：书云“文武多才”。秀气的文采配合强硬的手段，适合从事管理、公检法或企业高管。' });
  if (has(5) && has(1) && has(7)) interactions.push({ title: '智谋生财', type: 'good', desc: '天奸（5）逢 贵（1）福（7）：书云“财帛丰盈”。聪明才智用于正途，能通过智谋获得丰厚财富，亦为上命。' });
  if (has(7) && has(3) && has(10)) interactions.push({ title: '权福相生', type: 'good', desc: '天福（7）逢 权（3）刃（10）：书云“衣帛充足”。有权有势又有福，主仓库盈余，堆金积玉。' });
  if (has(12) && (has(3) || has(7))) interactions.push({ title: '福寿绵长', type: 'good', desc: '天寿（12）逢 权（3）或 福（7）：书云“宽宏大量”。主性格温良，福寿双全，一生安稳。' });

  if (has(1) && (has(2) || has(10) || has(9))) interactions.push({ title: '贵星受损', type: 'mixed', desc: '天贵星（1）逢 厄（2）/刃（10）/孤（9）：书云“朝堂折挫”。虽有贵气，但易遭小人嫉妒或身体有损，见贵而未为全吉。' });
  if (has(2) && has(10)) interactions.push({ title: '厄刃交加', type: 'bad', desc: '天厄星（2）逢 天刃星（10）：书云“疾病贫穷”。鬼道的暗疾加上畜道的刚烈，需特别注意身体健康和情绪管理，防手足损伤。' });
  if (has(5) && (has(3) || has(10))) interactions.push({ title: '奸权残忍', type: 'mixed', desc: '天奸（5）逢 权（3）或 刃（10）：书云“必为奸权残忍之人”。主心机深沉，手段强硬，虽然有能力，但恐缺德行，需修慈悲心。' });
  if (has(11) && (has(2) || has(4))) interactions.push({ title: '艺途多舛', type: 'bad', desc: '天艺星（11）逢 厄（2）或 破（4）：书云“艺举无成”。虽有才艺，但往往难以变现或怀才不遇，建议放低身段，先求生存。' });
  if (has(2) && (has(8) || has(9))) interactions.push({ title: '漂泊劳碌', type: 'bad', desc: '天厄星（2）逢 天驿（8）/天孤（9）：书云“风吹树叶水上浮萍”。主一生奔波，心猿意马，难安一处，宜修定力。' });
  if (has(4) && (has(8) || has(9) || has(2))) interactions.push({ title: '作事艰难', type: 'bad', desc: '天破（4）逢 驿（8）/孤（9）/厄（2）：书云“作事艰难”。主祖业耗散，浮浪东西，需防重重破败。' });
  if (has(6) && (has(2) || has(4) || has(8) || has(9))) interactions.push({ title: '多学少成', type: 'mixed', desc: '天文（6）逢 厄/破/驿/孤：书云“多学少成”。虽聪明伶俐，但恐不为文墨之辈，反为云游术士或手艺人。' });
  if (has(10) && (has(9) || has(2) || has(4))) interactions.push({ title: '形体残疾', type: 'bad', desc: '天刃（10）逢 孤/破/厄：书云“形体残疾”。胆大心粗，易招灾祸，需特别注意安全。' });

  return interactions;
};

const getLifeAdvice = (pillars) => {
  const dayStar = pillars[2].data;
  const monthStar = pillars[1].data;

  const counts = { BUDDHA: 0, IMMORTAL: 0, HUMAN: 0, ASURA: 0, GHOST: 0, ANIMAL: 0 };
  pillars.forEach(p => {
    Object.keys(REALMS).forEach(key => {
      if (p.data.realm.name === REALMS[key].name) counts[key]++;
    });
  });
  let maxRealm = 'HUMAN';
  let maxCount = 0;
  Object.keys(counts).forEach(key => {
    if (counts[key] > maxCount) {
      maxCount = counts[key];
      maxRealm = key;
    }
  });

  let loveAdvice = "";
  let friendAdvice = "";
  let coreAdvice = "";

  if (['天贵星', '天福星', '天文星', '天寿星'].includes(dayStar.name)) {
    loveAdvice = "您的日柱坐吉星（佛/仙道），预示配偶多为贤良或有福之人。建议寻找性格温和、有文化底蕴或家庭背景良好的伴侣。相处中宜以和为贵，您的婚姻往往是您后半生幸福的基石。";
  } else if (['天权星', '天奸星', '天刃星'].includes(dayStar.name)) {
    loveAdvice = "日柱坐权/刃/奸（人/修罗/畜道），主配偶性格强势或能力极强。建议寻找包容性强、能以柔克刚的伴侣，或者双方在事业上各自独立，避免家庭内部的权力争夺。";
  } else {
    loveAdvice = "日柱逢破/厄/驿/孤（鬼/畜/人道），感情路可能稍显波折或内心孤独。建议寻找成熟稳重、能给您带来安全感的伴侣。避免异地恋，多沟通内心感受，平淡是真。";
  }

  if (monthStar.realm.name === '佛道' || monthStar.realm.name === '仙道') {
    friendAdvice = "月柱入清净道，您在交友上贵精不贵多。适合结交有精神追求、志同道合的“君子之交”。远离酒肉朋友，他们会消耗您的灵气。";
  } else if (monthStar.realm.name === '人道' || monthStar.realm.name === '修罗') {
    friendAdvice = "月柱入入世道，您的社交圈子广，人脉即钱脉。适合结交各行各业的精英。但需防范利益之交背后的算计，保持三分清醒。";
  } else {
    friendAdvice = "月柱入劳碌道（鬼/畜），交友需谨慎，容易遇到消耗您能量的人。建议多结交长辈或导师型的人物（贵人），减少无效社交。";
  }

  switch (maxRealm) {
    case 'BUDDHA': coreAdvice = "【佛道重】您的慧根深厚，核心修养在于“慈悲与入世”。您容易出世清高，建议多参与社会公益，将智慧转化为造福大众的行动，避免陷入虚无主义。"; break;
    case 'IMMORTAL': coreAdvice = "【仙道重】您的灵性极高，核心修养在于“落地与专注”。您才华横溢但容易飘忽不定，建议选定一个领域深耕，将灵感具象化为作品，避免眼高手低。"; break;
    case 'HUMAN': coreAdvice = "【人道重】您的入世能力强，核心修养在于“知足与平衡”。您容易陷入名利的追逐中无法自拔，建议定期从繁忙中抽离，培养一种非功利的爱好，滋养心灵。"; break;
    case 'ASURA': coreAdvice = "【修罗道重】您的斗志旺盛，核心修养在于“忍辱与柔和”。您好胜心强，容易树敌，建议修炼“退一步海阔天空”的智慧，学习用柔和的方式解决冲突。"; break;
    case 'GHOST': coreAdvice = "【鬼道重】您的直觉敏锐，核心修养在于“布施与感恩”。您内心深处可能有匮乏感或焦虑感，建议通过物质或精神的布施来建立富足感，感恩当下。"; break;
    case 'ANIMAL': coreAdvice = "【畜道重】您的执行力强，核心修养在于“读书与明理”。您可能做事较冲动或随波逐流，建议多读圣贤书，提升认知维度，凡事三思而后行。"; break;
    default: coreAdvice = "您的命盘能量分布均衡，核心修养在于“中道”。不偏不倚，顺势而为。";
  }

  return { loveAdvice, friendAdvice, coreAdvice };
};

const generateAnalysis = (pillars, gender) => {
  const counts = { BUDDHA: 0, IMMORTAL: 0, HUMAN: 0, ASURA: 0, GHOST: 0, ANIMAL: 0 };
  const starsCount = {};
  pillars.forEach(p => {
    Object.keys(REALMS).forEach(key => {
      if (p.data.realm.name === REALMS[key].name) counts[key]++;
    });
    starsCount[p.data.id] = (starsCount[p.data.id] || 0) + 1;
  });

  let overallFate = "";
  let overallAdvice = "";
  const timeStar = pillars[3].data;
  const isTimeAuspicious = ['天贵星', '天福星', '天文星', '天寿星'].includes(timeStar.name);

  if (isTimeAuspicious && (counts.BUDDHA + counts.IMMORTAL >= 2)) {
    overallFate = "【上品格局】您的命盘根基深厚，晚运安详。四柱中吉星高照，主一生贵人多助，逢凶化吉，晚景令人羡慕。";
  } else if (counts.ASURA + counts.GHOST + counts.ANIMAL >= 3) {
    overallFate = "【需修行的命格】您的四柱中挑战较多（修罗/鬼/畜道星重），人生可能波折较大或内心冲突较强。但这正是灵魂选择来此磨练的课题。";
  } else {
    overallFate = "【中平格局】吉凶参半，运势起伏。虽有挑战，但也有救应。关键在于后天的选择与修养，切勿妄自菲薄。";
  }

  if (gender === 'male') {
    if (starsCount[8] >= 1 || starsCount[9] >= 1) overallAdvice += "【男命特别提示】古诀云“男怕孤驿凶”。您的命盘中出现天孤或天驿，暗示一生奔波或人际疏离，需特别注意积攒人脉和保持家庭稳定。";
  } else {
    if (starsCount[4] >= 1 || starsCount[10] >= 1 || starsCount[2] >= 1) overallAdvice += "【女命特别提示】古诀云“女怕破刃厄”。您的命盘中出现天破、天刃或天厄，需注意身体保养和财务规划，凡事以柔克刚为上。";
  }

  if (counts.GHOST >= 2) overallAdvice += " 此外，命盘中“鬼道”星较重，内心易感匮乏，建议多做公益。";
  else if (counts.ASURA >= 2) overallAdvice += " 此外，“修罗”星重者好胜心强，建议修身养性。";

  const fanZhongFindings = getFanZhongAnalysis(starsCount);
  const interactionFindings = getInteractionAnalysis(pillars);
  const lifeAdvice = getLifeAdvice(pillars);

  return { overallFate, overallAdvice, fanZhongFindings, interactionFindings, lifeAdvice };
};

// Calculate Score helper for Birth Guide
const calculateScore = (pillars, gender) => {
  let score = 0;
  const starCounts = {};
  const starIds = [];

  pillars.forEach(p => {
    const id = p.data.id;
    starIds.push(id);
    starCounts[id] = (starCounts[id] || 0) + 1;

    if ([1, 7, 12].includes(id)) score += 2; // Gui, Fu, Shou
    else if ([3, 6].includes(id)) score += 1.5; // Quan, Wen
    else if ([5, 8, 9].includes(id)) score -= 1; // Jian, Yi, Gu
    else if ([2, 4, 10].includes(id)) score -= 2; // E, Po, Ren
  });

  if (gender === 'male') {
    if (starCounts[8]) score -= 2; // Yi
    if (starCounts[9]) score -= 2; // Gu
  } else {
    if (starCounts[4]) score -= 2; // Po
    if (starCounts[10]) score -= 2; // Ren
    if (starCounts[2]) score -= 2; // E
  }

  if (starIds.includes(1) && starIds.includes(3)) score += 3; // Gui + Quan
  if (starIds.includes(1) && starIds.includes(7)) score += 3; // Gui + Fu
  if (starIds.includes(2) && starIds.includes(10)) score -= 5; // E + Ren (Bad)

  if (starCounts[2] >= 3) score += 5;
  if (starCounts[1] >= 2) score -= 1;

  return score;
};

// Common Pillar Calculation Function
const calculatePillarsFromDate = (dateStr, timeStr, gender) => {
  if (!window.Solar) throw new Error("Solar library not ready");
  const solarDate = new Date(dateStr);
  const lunar = window.Solar.fromDate(solarDate).getLunar();

  const lunarYearZhiIndex = lunar.getYearZhiIndex() + 1;
  const lunarMonth = Math.abs(lunar.getMonth());
  const lunarDay = lunar.getDay();

  const hourInt = parseInt(timeStr.split(':')[0], 10);
  const minuteInt = parseInt(timeStr.split(':')[1], 10);
  const timeBranchIndex = getTimeBranchIndex(hourInt);

  const isClockwise = gender === 'male';

  const yearPos = lunarYearZhiIndex;
  const monthPos = calculatePosition(yearPos, lunarMonth, isClockwise);
  const dayPos = calculatePosition(monthPos, lunarDay, isClockwise);
  const timePos = calculatePosition(dayPos, timeBranchIndex, isClockwise);

  let branchStartHour = (timeBranchIndex === 1) ? 23 : (timeBranchIndex - 2) * 2 + 1;
  let diffHours = hourInt - branchStartHour;
  if (hourInt === 0 && branchStartHour === 23) diffHours = 1;
  const minutesIntoBranch = diffHours * 60 + minuteInt;
  const timeSub = getTimeSubdivision(minutesIntoBranch);

  const pillars = [
    {
      id: 'year', title: '年柱', pos: yearPos, data: STARS[yearPos],
      age: '1-15岁', specific_analysis: STARS[yearPos].pos_meaning.year
    },
    {
      id: 'month', title: '月柱', pos: monthPos, data: STARS[monthPos],
      age: '16-30岁', specific_analysis: STARS[monthPos].pos_meaning.month
    },
    {
      id: 'day', title: '日柱', pos: dayPos, data: STARS[dayPos],
      age: '31-45岁', specific_analysis: STARS[dayPos].pos_meaning.day
    },
    {
      id: 'time', title: '时柱', pos: timePos, data: STARS[timePos],
      age: '46岁以后', specific_analysis: STARS[timePos].pos_meaning.hour
    },
  ];

  return {
    pillars,
    lunar: {
      year: `${lunar.getYearInGanZhi()}年 (${lunar.getYearShengXiao()})`,
      month: `${lunar.getMonthInChinese()}月`,
      day: `${lunar.getDayInChinese()}`,
      time: `${STARS[timeBranchIndex].branch}时`,
    },
    timeSub,
    timeBranchIndex
  };
};

// --- AI Interpretation Feature ---
// Function to call Gemini API
// Function to call Gemini API
const generateGeminiInterpretation = async (pillars, gender, lunarDate) => {
  const apiKey = import.meta.env.VITE_API_KEY;
  if (!apiKey) {
    return "⚠️ API Key missing. Please set VITE_API_KEY in your .env file.";
  }
  const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`;

  const pillarText = pillars.map((p, i) => {
    const title = ['年柱', '月柱', '日柱', '时柱'][i] || '未知';
    return `${title}: ${p.data.name} (${p.data.realm.name} - ${p.data.realm.trait})`;
  }).join('\n');

  const genderText = gender === 'male' ? '男' : '女';

  const prompt = `
    你是一位精通《达摩一掌经》和深谙人性的命理大师。
    请根据以下缘主的排盘信息，进行一次深度、温暖且富有洞察力的解读。
    
    【缘主信息】
    性别：${genderText}
    出生信息：${lunarDate.year} ${lunarDate.month} ${lunarDate.day} ${lunarDate.time}
    
    【四柱排盘】
    ${pillarText}
    
    【解读要求】
    1. **性格深层剖析**：不要只解释单星，要分析四柱星宿之间的化学反应（例如：年柱佛道+月柱修罗意味着什么内心的冲突？）。
    2. **一生运势流转**：结合年（根基）、月（事业/青年）、日（中年/配偶）、时（晚年/归宿）的顺序，讲述一个连贯的生命故事。
    3. **当下核心课题**：指出缘主目前可能面临的最大挑战是什么（基于星宿特性），并给出化解之道。
    4. **大师寄语**：给出一句充满智慧和力量的总结语。
    
    请用Markdown格式输出，保持语气平和、慈悲，既指出问题，又给出希望。不要过于宿命论，要强调修行的力量。
  `;

  try {
    const response = await fetch(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        contents: [{ parts: [{ text: prompt }] }]
      })
    });

    if (!response.ok) throw new Error('API request failed');
    const data = await response.json();
    return data.candidates[0].content.parts[0].text;
  } catch (error) {
    console.error("Gemini API Error:", error);
    return "大师正在闭关参禅，暂时无法连接（网络请求失败，请稍后再试）。";
  }
};

// --- Component ---

const OnePalmApp = () => {
  const [loading, setLoading] = useState(false);
  const [lunarLibReady, setLunarLibReady] = useState(true);

  // Mode State
  const [mode, setMode] = useState('analysis'); // 'analysis', 'guide', or 'relation'

  // Analysis State
  const [gender, setGender] = useState('male');
  const [birthDate, setBirthDate] = useState('1990-06-15');
  const [birthTime, setBirthTime] = useState('12:00');
  const [result, setResult] = useState(null);
  const [activeTab, setActiveTab] = useState('analysis');

  // Guide State
  const [guideDueDate, setGuideDueDate] = useState(new Date().toISOString().split('T')[0]);
  const [guideGender, setGuideGender] = useState('male');
  const [guideResults, setGuideResults] = useState([]);
  const [guideLoading, setGuideLoading] = useState(false);

  // Relation State (New!)
  const [relName, setRelName] = useState('对方');
  const [relGender, setRelGender] = useState('female');
  const [relDate, setRelDate] = useState('1992-08-20');
  const [relTime, setRelTime] = useState('12:00');
  const [relType, setRelType] = useState('partner'); // partner, friend, family, colleague
  const [relationResult, setRelationResult] = useState(null);

  // AI State (Main Analysis)
  const [aiAnalysis, setAiAnalysis] = useState(null);
  const [isAiLoading, setIsAiLoading] = useState(false);

  // AI State (Guide Modal)
  const [activeGuideAi, setActiveGuideAi] = useState(null); // { loading, content } or null

  // UI State - Print Handling
  const [isPrinting, setIsPrinting] = useState(false);

  // Removed useEffect for script loading as we import it directly

  const handleAiAnalysis = async () => {
    if (!result) return;
    setIsAiLoading(true);
    setActiveTab('ai'); // Switch to AI tab automatically
    const analysis = await generateGeminiInterpretation(result.pillars, result.gender, result.lunar);
    setAiAnalysis(analysis);
    setIsAiLoading(false);
  };

  const handleGuideAi = async (item) => {
    setActiveGuideAi({ loading: true, content: null, item });
    // Pass guide gender explicitly
    const analysis = await generateGeminiInterpretation(item.pillarObjects, guideGender, item.lunarDetail);
    setActiveGuideAi({ loading: false, content: analysis, item });
  };

  const handlePrint = () => {
    setIsPrinting(true);
    setTimeout(() => {
      window.print();
      setIsPrinting(false);
    }, 50);
  };

  // ... HandGrid Component
  const HandGrid = ({ activePositions, printMode = false }) => {
    const palmLayout = [
      { id: 6, label: '巳 仙' }, { id: 7, label: '午 佛' }, { id: 8, label: '未 鬼' }, { id: 9, label: '申 人' },
      { id: 5, label: '辰 修' }, { id: null }, { id: null }, { id: 10, label: '酉 畜' },
      { id: 4, label: '卯 畜' }, { id: null }, { id: null }, { id: 11, label: '戌 修' },
      { id: 3, label: '寅 人' }, { id: 2, label: '丑 鬼' }, { id: 1, label: '子 佛' }, { id: 12, label: '亥 仙' },
    ];

    const containerClasses = printMode
      ? "relative w-48 aspect-square p-2 border-[4px] border-stone-800 rounded-xl select-none mx-auto mb-4 grayscale"
      : "relative w-full h-auto min-h-[24rem] max-w-sm mx-auto p-4 bg-[#f4eebf] rounded-3xl border-[6px] border-[#e6dca3] shadow-inner select-none";

    const cellClasses = (isActive, realm) => {
      if (printMode) {
        return `relative flex flex-col items-center justify-center rounded border text-xs font-bold ${isActive ? 'border-black bg-stone-100 text-black' : 'border-stone-300 text-stone-300'}`;
      }
      return `relative flex flex-col items-center justify-center rounded-xl border-2 transition-transform duration-300 min-h-[5rem] ${isActive ? `${realm.bg} ${realm.color} ${realm.border} shadow-lg scale-105 z-10` : 'bg-white/50 border-stone-200/50 text-stone-400'}`;
    };

    return (
      <div className={containerClasses}>
        {!printMode && (
          <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
            <Hand size={240} className="text-stone-500" />
          </div>
        )}
        <div className="grid grid-cols-4 gap-2 h-full text-center">
          {palmLayout.map((cell, idx) => {
            if (!cell.id) return <div key={idx} />;
            const matches = activePositions.filter(p => p.pos === cell.id);
            const isActive = matches.length > 0;
            const star = STARS[cell.id];

            return (
              <div key={idx} className={cellClasses(isActive, star.realm)}>
                <div className={printMode ? "scale-90" : "text-sm md:text-base"}>{cell.label.split(' ')[0]}</div>
                {(!printMode || isActive) && <div className={`scale-90 opacity-80 ${printMode ? 'text-[8px]' : 'text-[10px]'}`}>{star.name}</div>}
                {isActive && (
                  <div className={`absolute -top-2 -right-1 flex flex-col items-center justify-center font-bold leading-tight z-20 ${printMode ? 'text-[8px] bg-black text-white px-1 rounded' : 'bg-red-600 text-white rounded-lg px-1.5 py-0.5 text-[10px] shadow-md ring-1 ring-white'}`}>
                    {matches.map((m, i) => (
                      <span key={i} className="block">{m.label}</span>
                    ))}
                  </div>
                )}
              </div>
            );
          })}
        </div>
      </div>
    );
  };

  const calculateDestiny = () => {
    if (!lunarLibReady || !window.Solar) {
      alert("核心算法库正在加载，请稍等1-2秒后再试...");
      return;
    }

    try {
      const calcResult = calculatePillarsFromDate(birthDate, birthTime, gender);

      const shi = getShi(calcResult.pillars[0].pos, Math.abs(window.Solar.fromDate(new Date(birthDate)).getLunar().getMonth()));
      const lunarDay = window.Solar.fromDate(new Date(birthDate)).getLunar().getDay();
      const dayStarName = DAY_STARS[lunarDay];
      const dayStarDesc = DAY_STAR_MEANING[dayStarName];
      const analysis = generateAnalysis(calcResult.pillars, gender);

      setResult({
        ...calcResult,
        analysis,
        gender,
        advanced: {
          timeSub: calcResult.timeSub,
          shi,
          dayStar: { name: dayStarName, desc: dayStarDesc }
        }
      });
      setAiAnalysis(null);
      setActiveTab('analysis');
    } catch (e) {
      console.error(e);
      alert("计算出错，请检查输入日期。");
    }
  };

  const calculateRelation = () => {
    if (!result) {
      alert("请先完成您自己的排盘分析，再进行合盘。");
      setMode('analysis');
      return;
    }
    try {
      const targetCalc = calculatePillarsFromDate(relDate, relTime, relGender);

      // Analyze Relationship
      // 1. Year Harm (Zodiac)
      const myYearId = result.pillars[0].pos;
      const targetYearId = targetCalc.pillars[0].pos;
      const harm = ZODIAC_HARM[myYearId];
      const isHarm = harm && harm.target === targetYearId;

      // 2. Missing Star Complement
      const myStars = new Set(result.pillars.map(p => p.data.id));
      const targetStars = new Set(targetCalc.pillars.map(p => p.data.id));
      const complements = [];
      if (!myStars.has(1) && targetStars.has(1)) complements.push('对方带有【天贵星】，可补您贵人之气');
      if (!myStars.has(3) && targetStars.has(3)) complements.push('对方带有【天权星】，可助您事业决策');
      if (!myStars.has(7) && targetStars.has(7)) complements.push('对方带有【天福星】，可增您福报财源');
      if (!myStars.has(6) && targetStars.has(6)) complements.push('对方带有【天文星】，可添您灵秀智慧');

      // 3. Influence Type
      let influenceScore = 0;
      const targetCounts = { BUDDHA: 0, HUMAN: 0, GHOST: 0, ANIMAL: 0 };
      targetCalc.pillars.forEach(p => {
        if (p.data.realm.name === '佛道' || p.data.realm.name === '仙道') { influenceScore += 2; targetCounts.BUDDHA++; }
        if (p.data.realm.name === '人道' || p.data.realm.name === '修罗') { influenceScore += 1; targetCounts.HUMAN++; }
        if (p.data.realm.name === '鬼道') { influenceScore -= 1; targetCounts.GHOST++; }
        if (p.data.realm.name === '畜道') { influenceScore -= 1; targetCounts.ANIMAL++; }
      });

      let influenceType = "中性影响";
      if (targetCounts.BUDDHA >= 2) influenceType = "精神贵人型 (提升您的灵性与福报)";
      else if (targetCounts.HUMAN >= 2) influenceType = "事业助力型 (在现实层面推动您)";
      else if (targetCounts.GHOST >= 2) influenceType = "需要包容型 (对方可能内心敏感，需您多担待)";
      else if (targetCounts.ANIMAL >= 2) influenceType = "直率冲动型 (对方执行力强但可能消耗您的耐心)";

      setRelationResult({
        target: targetCalc,
        analysis: {
          isHarm,
          harmDesc: isHarm ? harm.desc : '年柱生肖无冲害，基础缘分平稳',
          complements,
          influenceType,
          influenceScore
        }
      });

    } catch (e) {
      console.error(e);
      alert("合盘计算出错");
    }
  };

  // --- New: Birth Guide Calculation (retained) ---
  const calculateBirthGuide = () => {
    if (!lunarLibReady || !window.Solar) return;
    setGuideLoading(true);
    setGuideResults([]);

    setTimeout(() => {
      const results = [];
      const startDate = new Date(guideDueDate);
      startDate.setDate(startDate.getDate() - 30);

      const isClockwise = guideGender === 'male';

      for (let i = 0; i <= 60; i++) {
        const currentDate = new Date(startDate);
        currentDate.setDate(startDate.getDate() + i);
        const solar = window.Solar.fromDate(currentDate);
        const lunar = solar.getLunar();

        const lunarYearZhiIndex = lunar.getYearZhiIndex() + 1;
        const lunarMonth = Math.abs(lunar.getMonth());
        const lunarDay = lunar.getDay();

        for (let h = 1; h <= 12; h++) {
          const timeBranchIndex = h;

          const yearPos = lunarYearZhiIndex;
          const monthPos = calculatePosition(yearPos, lunarMonth, isClockwise);
          const dayPos = calculatePosition(monthPos, lunarDay, isClockwise);
          const timePos = calculatePosition(dayPos, timeBranchIndex, isClockwise);

          const pillars = [
            { id: 'year', data: STARS[yearPos] },
            { id: 'month', data: STARS[monthPos] },
            { id: 'day', data: STARS[dayPos] },
            { id: 'time', data: STARS[timePos] }
          ];

          const score = calculateScore(pillars, guideGender);
          const timeLabel = STARS[timeBranchIndex].branch + "时";
          const timeRange = TIME_RANGES[timeBranchIndex];
          const solarStr = solar.toString();

          const starCounts = {};
          pillars.forEach(p => starCounts[p.data.id] = (starCounts[p.data.id] || 0) + 1);
          const fanZhong = Object.keys(starCounts).filter(id => starCounts[id] >= 2).map(id => STARS[id].name + "犯重");

          results.push({
            dateStr: solarStr,
            lunarStr: `${lunar.getMonthInChinese()}月${lunar.getDayInChinese()}`,
            timeLabel,
            timeRange,
            score,
            pillars: pillars.map(p => p.data.name),
            pillarObjects: pillars,
            lunarDetail: { year: lunar.getYearInGanZhi(), month: lunar.getMonthInChinese(), day: lunar.getDayInChinese(), time: timeLabel },
            keyStar: pillars[3].data.name,
            fanZhong
          });
        }
      }
      const topResults = results.sort((a, b) => b.score - a.score).slice(0, 12);
      setGuideResults(topResults);
      setGuideLoading(false);
    }, 100);
  };

  if (loading) return <div className="flex items-center justify-center h-screen bg-[#fdfbf7] text-stone-500 font-serif tracking-widest">天机运筹中...</div>;

  return (
    <>
      <style>{`
        .print-only { display: none; }
        @media print {
          html, body, #root { height: auto !important; overflow: visible !important; position: static !important; margin: 0 !important; padding: 0 !important; display: block !important; }
          @page { size: A4; margin: 15mm; }
          body { background: white !important; color: black !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
          .no-print { display: none !important; }
          .print-only { display: block !important; width: 100% !important; height: auto !important; position: static !important; visibility: visible !important; }
          .hidden { display: block !important; }
        }
      `}</style>

      <div className="min-h-screen bg-[#fdfbf7] font-serif text-stone-800 p-6 md:p-12 no-print">
        <div className="max-w-6xl mx-auto space-y-12">

          <header className="text-center space-y-4">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-stone-800 text-amber-50 shadow-xl mb-2">
              <Book size={32} />
            </div>
            <h1 className="text-4xl md:text-5xl font-bold tracking-[0.2em] text-stone-900">达摩一掌经</h1>

            {/* Mode Switcher */}
            <div className="flex flex-wrap justify-center gap-4 mt-4">
              <button onClick={() => setMode('analysis')} className={`px-6 py-2 rounded-full font-bold transition-all ${mode === 'analysis' ? 'bg-stone-800 text-amber-50 shadow-lg' : 'bg-stone-200 text-stone-500 hover:bg-stone-300'}`}>排盘分析</button>
              <button onClick={() => setMode('relation')} className={`px-6 py-2 rounded-full font-bold transition-all ${mode === 'relation' ? 'bg-stone-800 text-amber-50 shadow-lg' : 'bg-stone-200 text-stone-500 hover:bg-stone-300'}`}>人际合盘</button>
              <button onClick={() => setMode('guide')} className={`px-6 py-2 rounded-full font-bold transition-all ${mode === 'guide' ? 'bg-stone-800 text-amber-50 shadow-lg' : 'bg-stone-200 text-stone-500 hover:bg-stone-300'}`}>出生择吉</button>
              {result && (
                <button onClick={handlePrint} disabled={isPrinting} className={`px-6 py-2 rounded-full font-bold transition-all border border-stone-300 text-stone-600 flex items-center gap-2 ${isPrinting ? 'bg-stone-100 cursor-wait' : 'bg-white hover:bg-stone-100'}`} title="打印A4报告">
                  <Printer size={18} /> {isPrinting ? '正在调起...' : '打印报告'}
                </button>
              )}
            </div>
          </header>

          {mode === 'analysis' ? (
            <>
              {/* ANALYSIS MODE INPUT */}
              <div className="bg-white rounded-2xl shadow-xl border border-stone-200 overflow-hidden w-full mx-auto transform transition-all hover:shadow-2xl no-print">
                <div className="bg-stone-50 px-8 py-4 border-b border-stone-100 flex items-center gap-2">
                  <Compass className="text-amber-600" size={20} />
                  <h2 className="font-bold text-stone-700 tracking-wide">排盘信息录入</h2>
                </div>
                <div className="p-8 md:p-10">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
                    <div className="space-y-3">
                      <label className="text-xs font-bold text-stone-400 uppercase tracking-widest">出生日期 (公历)</label>
                      <input type="date" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} className="w-full p-4 bg-stone-50 border border-stone-200 rounded-xl text-stone-800 outline-none focus:ring-2 focus:ring-amber-200 font-mono text-lg" />
                    </div>
                    <div className="space-y-3">
                      <label className="text-xs font-bold text-stone-400 uppercase tracking-widest">出生时间</label>
                      <input type="time" value={birthTime} onChange={(e) => setBirthTime(e.target.value)} className="w-full p-4 bg-stone-50 border border-stone-200 rounded-xl text-stone-800 outline-none focus:ring-2 focus:ring-amber-200 font-mono text-lg" />
                    </div>
                    <div className="space-y-3">
                      <label className="text-xs font-bold text-stone-400 uppercase tracking-widest">性别 (决定顺逆)</label>
                      <div className="flex gap-4">
                        <button onClick={() => setGender('male')} className={`flex-1 py-4 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${gender === 'male' ? 'bg-stone-800 text-amber-50 shadow-lg' : 'bg-stone-100 text-stone-400'}`}><Sun size={18} /> 男</button>
                        <button onClick={() => setGender('female')} className={`flex-1 py-4 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${gender === 'female' ? 'bg-stone-800 text-amber-50 shadow-lg' : 'bg-stone-100 text-stone-400'}`}><RotateCcw size={18} /> 女</button>
                      </div>
                    </div>
                  </div>
                  <div className="mt-10 text-center">
                    <button onClick={calculateDestiny} className="px-12 py-4 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-full shadow-lg hover:shadow-amber-200/50 transform transition-all active:scale-95 text-lg tracking-widest flex items-center gap-2 mx-auto">
                      <Sparkles size={20} /> 开始推演
                    </button>
                  </div>
                </div>
              </div>

              {/* ANALYSIS RESULTS */}
              {result && (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
                  {/* Left Column */}
                  <div className="lg:col-span-5 space-y-8">
                    <div className="bg-white p-8 rounded-3xl shadow-xl border border-stone-100 text-center relative overflow-hidden">
                      <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-amber-200 via-amber-400 to-amber-200"></div>
                      <h3 className="text-xl font-bold text-stone-800 mb-6 flex items-center justify-center gap-2"><Hand size={20} className="text-amber-600" /> 掌上排盘</h3>
                      <HandGrid activePositions={result.pillars.map((p, i) => ({ pos: p.pos, label: ['年', '月', '日', '时'][i] }))} />
                      <div className="mt-6 text-sm text-stone-500 font-mono bg-stone-50 py-2 px-4 rounded-full inline-block">{result.lunar.year} · {result.lunar.month} · {result.lunar.day} · {result.lunar.time}</div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      {result.pillars.map((p, i) => (
                        <div key={i} className={`p-5 rounded-2xl border-2 flex flex-col items-center text-center transition-all hover:scale-105 ${p.data.realm.bg} ${p.data.realm.border}`}>
                          <span className="text-[10px] font-bold uppercase tracking-widest opacity-50 mb-1">{p.title}</span>
                          <span className={`text-3xl mb-2 ${p.data.realm.color}`}>{p.data.realm.icon}</span>
                          <span className="font-bold text-stone-900 mb-1">{p.data.name}</span>
                          <span className={`text-[10px] px-2 py-0.5 rounded-full bg-white/60 ${p.data.realm.color}`}>{p.data.realm.name}</span>
                        </div>
                      ))}
                    </div>
                    {!aiAnalysis && !isAiLoading && (
                      <button onClick={handleAiAnalysis} className="w-full py-4 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold rounded-2xl shadow-lg hover:shadow-purple-200/50 transform transition-all active:scale-95 flex items-center justify-center gap-3">
                        <Bot size={24} /> ✨ AI 大师深度解盘
                      </button>
                    )}
                  </div>

                  {/* Right Column */}
                  <div className="lg:col-span-7">
                    <div className="bg-white rounded-3xl shadow-xl border border-stone-200 overflow-hidden min-h-[600px] flex flex-col">
                      <div className="flex border-b border-stone-100 p-2 gap-2 bg-stone-50/50 overflow-x-auto">
                        {['analysis', 'chart', 'advice', 'ai'].map((tab) => (
                          <button key={tab} onClick={() => setActiveTab(tab)} disabled={tab === 'ai' && !aiAnalysis && !isAiLoading} className={`flex-1 py-3 px-4 rounded-xl text-sm font-bold tracking-wide transition-all whitespace-nowrap ${activeTab === tab ? 'bg-white text-stone-900 shadow-md ring-1 ring-stone-200' : 'text-stone-400 hover:text-stone-600'} ${(tab === 'ai' && !aiAnalysis && !isAiLoading) ? 'opacity-50 cursor-not-allowed hidden md:block' : ''}`}>
                            <div className="flex items-center justify-center gap-2">
                              {tab === 'analysis' ? '综合运势' : tab === 'chart' ? '四柱详解' : tab === 'advice' ? '人生建议' : '🤖 AI 大师解读'}
                            </div>
                          </button>
                        ))}
                      </div>

                      {/* ANALYSIS TAB CONTENT */}
                      <div className="p-8 md:p-10 flex-grow bg-white">
                        {activeTab === 'analysis' && (
                          <div className="space-y-10 animate-in fade-in duration-500">
                            <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-8 rounded-2xl border border-amber-100">
                              <h3 className="text-xl font-bold text-amber-900 mb-4 flex items-center gap-2"><Sparkles size={20} /> 命格总评</h3>
                              <p className="text-stone-700 leading-loose text-lg font-medium">{result.analysis.overallFate}</p>

                              <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 pt-6 border-t border-amber-200/50">
                                <div className="bg-white/60 p-3 rounded-lg border border-amber-100 shadow-sm">
                                  <div className="flex items-center gap-2 text-amber-800 font-bold text-sm mb-1"><Clock size={14} /> 时分三刻</div>
                                  <div className="font-bold text-stone-800">{result.advanced.timeSub.label}</div>
                                  <div className="text-xs text-stone-500 mt-1">{result.advanced.timeSub.desc}</div>
                                </div>
                                <div className="bg-white/60 p-3 rounded-lg border border-amber-100 shadow-sm">
                                  <div className="flex items-center gap-2 text-amber-800 font-bold text-sm mb-1"><Gem size={14} /> 年月逢识</div>
                                  <div className="font-bold text-stone-800">{result.advanced.shi.name}</div>
                                  <div className="text-xs text-stone-500 mt-1">{result.advanced.shi.desc}</div>
                                </div>
                                <div className="bg-white/60 p-3 rounded-lg border border-amber-100 shadow-sm">
                                  <div className="flex items-center gap-2 text-amber-800 font-bold text-sm mb-1"><Calendar size={14} /> 日遇吉凶</div>
                                  <div className="font-bold text-stone-800">{result.advanced.dayStar.name}</div>
                                  <div className="text-xs text-stone-500 mt-1">{result.advanced.dayStar.desc}</div>
                                </div>
                              </div>
                              {result.analysis.overallAdvice && (
                                <div className="mt-6 pt-6 border-t border-amber-200/50 text-amber-800 text-sm leading-relaxed">
                                  <span className="font-bold bg-amber-200 text-amber-900 px-2 py-1 rounded mr-2">重点提示</span>{result.analysis.overallAdvice}
                                </div>
                              )}
                            </div>

                            {(result.analysis.fanZhongFindings.length > 0 || result.analysis.interactionFindings.length > 0) && (
                              <div className="bg-purple-50 p-6 rounded-2xl border border-purple-100">
                                <h3 className="text-lg font-bold text-purple-900 mb-4 flex items-center gap-2"><Star size={18} className="fill-purple-600 text-purple-600" /> 命盘格局·特别断语</h3>
                                <div className="space-y-3">
                                  {result.analysis.fanZhongFindings.map((f, i) => (
                                    <div key={`fz-${i}`} className="bg-white p-3 rounded-xl border border-purple-100 shadow-sm flex gap-3 items-start">
                                      <span className="bg-purple-100 text-purple-800 text-xs font-bold px-2 py-1 rounded shrink-0 mt-0.5">犯重: {f.star} x {f.count}</span>
                                      <span className="text-stone-700 text-sm leading-relaxed">{f.desc}</span>
                                    </div>
                                  ))}
                                  {result.analysis.interactionFindings.map((f, i) => (
                                    <div key={`in-${i}`} className="bg-white p-3 rounded-xl border border-purple-100 shadow-sm flex gap-3 items-start">
                                      <span className={`text-xs font-bold px-2 py-1 rounded shrink-0 mt-0.5 ${f.type === 'good' ? 'bg-emerald-100 text-emerald-800' : f.type === 'bad' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'}`}>{f.title}</span>
                                      <span className="text-stone-700 text-sm leading-relaxed">{f.desc}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>
                            )}

                            <div className="space-y-6">
                              <h3 className="text-lg font-bold text-stone-800 pl-4 border-l-4 border-stone-800">人生流年轨迹</h3>
                              <div className="space-y-8 relative before:absolute before:left-4 before:top-2 before:bottom-2 before:w-0.5 before:bg-stone-100">
                                {result.pillars.map((pillar, idx) => (
                                  <div key={idx} className="relative pl-12">
                                    <div className={`absolute left-[11px] top-1 w-6 h-6 rounded-full border-4 border-white shadow-sm z-10 ${pillar.data.realm.bg.replace('bg-', 'bg-')} flex items-center justify-center`}>
                                      <div className={`w-2 h-2 rounded-full ${pillar.data.realm.color.replace('text-', 'bg-')}`}></div>
                                    </div>
                                    <div>
                                      <div className="flex items-center gap-3 mb-2">
                                        <span className="text-stone-900 font-bold text-lg">{pillar.data.name}</span>
                                        <span className="text-stone-500 text-sm">({pillar.title})</span>
                                        <span className="text-stone-400 text-xs uppercase tracking-wider font-bold border border-stone-200 px-2 py-0.5 rounded-full">{pillar.age}</span>
                                      </div>
                                      <div className="bg-stone-50 p-6 rounded-2xl border border-stone-100 text-stone-600 leading-relaxed hover:bg-stone-100/50 transition-colors">
                                        <div className="flex gap-2 mb-2"><MapPin size={16} className="text-amber-600 mt-1" /><strong>宫位断语：</strong></div>
                                        {pillar.specific_analysis}
                                      </div>
                                    </div>
                                  </div>
                                ))}
                              </div>
                            </div>
                          </div>
                        )}
                        {activeTab === 'chart' && (
                          <div className="space-y-8 animate-in fade-in duration-500">
                            {result.pillars.map((pillar, idx) => (
                              <div key={idx} className="group relative pl-8 pb-8 border-l border-stone-200 last:border-0 last:pb-0">
                                <div className={`absolute -left-[21px] top-0 w-10 h-10 rounded-full border-4 border-white shadow-md flex items-center justify-center ${pillar.data.realm.bg} ${pillar.data.realm.color}`}>{idx + 1}</div>
                                <div className="bg-stone-50 rounded-2xl p-6 border border-stone-100 group-hover:shadow-md transition-all group-hover:bg-white group-hover:border-stone-200">
                                  <div className="flex justify-between items-start mb-4">
                                    <div>
                                      <h4 className="text-xl font-bold text-stone-900">{pillar.data.name}</h4>
                                      <span className={`text-xs font-bold px-2 py-1 rounded mt-2 inline-block ${pillar.data.realm.bg} ${pillar.data.realm.color}`}>{pillar.data.realm.name} · {pillar.data.realm.trait}</span>
                                    </div>
                                    <div className="text-right"><div className="text-xs text-stone-400 uppercase tracking-widest mb-1">{pillar.title}</div><div className="text-sm font-bold text-stone-600">{pillar.data.archetype}</div></div>
                                  </div>
                                  <div className="relative pl-4 mb-4 border-l-2 border-amber-200">
                                    <p className="italic text-stone-600 font-serif leading-loose text-sm">“{pillar.data.poem}”</p>
                                  </div>
                                  <div className="space-y-4 mt-4 pt-4 border-t border-stone-100">
                                    <div className="text-sm text-stone-600 leading-relaxed">
                                      <div className="flex gap-2 mb-1 text-stone-800 font-bold items-center"><Scroll size={14} /> <span>星宿本义：</span></div>
                                      {pillar.data.general_analysis}
                                    </div>
                                    <div className="bg-amber-50/50 p-3 rounded-lg text-sm text-stone-600 leading-relaxed">
                                      <div className="flex gap-2 mb-1 text-amber-800 font-bold items-center"><User size={14} /> <span>性别差异 ({result.gender === 'male' ? '男' : '女'})：</span></div>
                                      {pillar.data.gender_specific[result.gender]}
                                    </div>
                                    <div className="text-sm text-stone-600 leading-relaxed">
                                      <div className="flex gap-2 mb-1 text-stone-800 font-bold items-center"><MapPin size={14} /> <span>落宫详解 ({pillar.title.split(' ')[0]})：</span></div>
                                      {pillar.specific_analysis}
                                    </div>
                                  </div>
                                </div>
                              </div>
                            ))}
                          </div>
                        )}
                        {activeTab === 'advice' && (
                          <div className="space-y-8 animate-in fade-in duration-500">
                            <div className="bg-gradient-to-br from-teal-50 to-emerald-50 p-6 rounded-2xl border border-teal-100 shadow-sm">
                              <h3 className="text-xl font-bold text-teal-900 mb-4 flex items-center gap-2"><Users size={20} /> 综合人生指南</h3>
                              <div className="space-y-4">
                                <div className="bg-white/70 p-4 rounded-xl border border-teal-100/50"><h4 className="font-bold text-teal-800 mb-2 flex items-center gap-2"><Heart size={16} /> 择偶建议</h4><p className="text-sm text-teal-900 leading-relaxed">{result.analysis.lifeAdvice.loveAdvice}</p></div>
                                <div className="bg-white/70 p-4 rounded-xl border border-teal-100/50"><h4 className="font-bold text-teal-800 mb-2 flex items-center gap-2"><Users size={16} /> 交友之道</h4><p className="text-sm text-teal-900 leading-relaxed">{result.analysis.lifeAdvice.friendAdvice}</p></div>
                                <div className="bg-white/70 p-4 rounded-xl border border-teal-100/50"><h4 className="font-bold text-teal-800 mb-2 flex items-center gap-2"><Flower size={16} /> 核心修养</h4><p className="text-sm text-teal-900 leading-relaxed">{result.analysis.lifeAdvice.coreAdvice}</p></div>
                              </div>
                            </div>
                            <div className="grid gap-6 md:grid-cols-2">
                              {result.pillars.map((pillar, idx) => (
                                <div key={idx} className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
                                  <h4 className="font-bold text-stone-800 mb-4 pb-2 border-b border-stone-100 flex items-center justify-between"><span>{pillar.data.name}</span><span className="text-xs text-stone-400 font-normal">针对{pillar.title.split(' ')[0]}</span></h4>
                                  <p className="text-sm text-stone-600 leading-loose">{pillar.data.advice}</p>
                                </div>
                              ))}
                            </div>
                          </div>
                        )}
                        {activeTab === 'ai' && (
                          <div className="space-y-8 animate-in fade-in duration-500">
                            {isAiLoading ? (
                              <div className="flex flex-col items-center justify-center py-20 space-y-4 text-purple-600">
                                <div className="animate-spin"><RotateCcw size={40} /></div>
                                <p className="text-lg font-bold animate-pulse">大师正在闭关参禅，为您深度推演...</p>
                              </div>
                            ) : aiAnalysis ? (
                              <div className="bg-gradient-to-br from-purple-50 to-indigo-50 p-8 rounded-3xl border border-purple-100 shadow-inner">
                                <h3 className="text-2xl font-bold text-purple-900 mb-6 flex items-center gap-3 border-b border-purple-200 pb-4"><Bot size={32} className="text-purple-600" /> 大师亲批 · 深度命理解读</h3>
                                <div className="prose prose-purple max-w-none text-stone-700 leading-loose font-medium whitespace-pre-wrap">{aiAnalysis}</div>
                              </div>
                            ) : (
                              <div className="flex flex-col items-center justify-center py-20 text-stone-400"><Bot size={60} className="mb-4 opacity-20" /><p>点击左侧“AI 大师深度解盘”按钮开始</p></div>
                            )}
                          </div>
                        )}
                      </div>
                    </div>
                  </div>
                </div>
              )}
            </>
          ) : mode === 'relation' ? (
            // RELATION MODE
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8 no-print">
              <div className="bg-white rounded-2xl shadow-xl border border-stone-200 overflow-hidden w-full mx-auto">
                <div className="bg-rose-50 px-8 py-4 border-b border-rose-100 flex items-center gap-2">
                  <UserPlus className="text-rose-600" size={20} />
                  <h2 className="font-bold text-rose-800 tracking-wide">人际合盘 · 缘分推演</h2>
                </div>
                <div className="p-8 md:p-10">
                  {!result ? (
                    <div className="text-center text-stone-500 py-10">
                      <AlertCircle size={40} className="mx-auto mb-4 opacity-50" />
                      <p>请先在“排盘分析”中输入您的信息，作为合盘的主体。</p>
                      <button onClick={() => setMode('analysis')} className="mt-4 text-rose-600 font-bold underline">去排盘</button>
                    </div>
                  ) : (
                    <>
                      <div className="bg-stone-50 p-4 rounded-xl mb-8 flex items-center gap-4 border border-stone-200">
                        <div className="bg-stone-800 text-white w-10 h-10 rounded-full flex items-center justify-center font-bold shrink-0">我</div>
                        <div className="text-sm text-stone-600">
                          <span className="font-bold block text-stone-800">我的四柱:</span>
                          {result.pillars.map(p => p.data.name).join(' / ')}
                        </div>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-3">
                          <label className="text-xs font-bold text-stone-400 uppercase tracking-widest">对方称呼 / 关系</label>
                          <div className="flex gap-2">
                            <input type="text" value={relName} onChange={(e) => setRelName(e.target.value)} className="flex-1 p-4 bg-stone-50 border border-stone-200 rounded-xl text-stone-800 outline-none focus:ring-2 focus:ring-rose-200" placeholder="如：李四" />
                            <select value={relType} onChange={(e) => setRelType(e.target.value)} className="p-4 bg-stone-50 border border-stone-200 rounded-xl text-stone-800 outline-none">
                              <option value="partner">伴侣</option>
                              <option value="colleague">事业伙伴</option>
                              <option value="friend">朋友</option>
                              <option value="family">家人</option>
                            </select>
                          </div>
                        </div>
                        <div className="space-y-3">
                          <label className="text-xs font-bold text-stone-400 uppercase tracking-widest">对方性别</label>
                          <div className="flex gap-4">
                            <button onClick={() => setRelGender('male')} className={`flex-1 py-4 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${relGender === 'male' ? 'bg-rose-800 text-white shadow-lg' : 'bg-stone-100 text-stone-400'}`}><Sun size={18} /> 男</button>
                            <button onClick={() => setRelGender('female')} className={`flex-1 py-4 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${relGender === 'female' ? 'bg-rose-800 text-white shadow-lg' : 'bg-stone-100 text-stone-400'}`}><RotateCcw size={18} /> 女</button>
                          </div>
                        </div>
                        <div className="space-y-3">
                          <label className="text-xs font-bold text-stone-400 uppercase tracking-widest">对方生日</label>
                          <input type="date" value={relDate} onChange={(e) => setRelDate(e.target.value)} className="w-full p-4 bg-stone-50 border border-stone-200 rounded-xl text-stone-800 outline-none focus:ring-2 focus:ring-rose-200 font-mono text-lg" />
                        </div>
                        <div className="space-y-3">
                          <label className="text-xs font-bold text-stone-400 uppercase tracking-widest">对方时辰</label>
                          <input type="time" value={relTime} onChange={(e) => setRelTime(e.target.value)} className="w-full p-4 bg-stone-50 border border-stone-200 rounded-xl text-stone-800 outline-none focus:ring-2 focus:ring-rose-200 font-mono text-lg" />
                        </div>
                      </div>
                      <div className="mt-8 text-center">
                        <button onClick={calculateRelation} className="px-12 py-4 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-full shadow-lg hover:shadow-rose-200/50 transform transition-all active:scale-95 text-lg tracking-widest flex items-center gap-2 mx-auto">
                          <ArrowRightLeft size={20} /> 开始合盘
                        </button>
                      </div>
                    </>
                  )}
                </div>
              </div>

              {relationResult && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  {/* Target Chart Preview */}
                  <div className="bg-white p-6 rounded-2xl shadow-md border border-stone-100">
                    <h3 className="font-bold text-stone-800 mb-4 flex items-center gap-2 border-b border-stone-100 pb-2">
                      <User size={18} /> {relName} 的命盘
                    </h3>
                    <div className="grid grid-cols-2 gap-3">
                      {relationResult.target.pillars.map((p, i) => (
                        <div key={i} className={`p-3 rounded-xl border text-center ${p.data.realm.bg} ${p.data.realm.border}`}>
                          <span className="text-[10px] opacity-50 block">{p.title}</span>
                          <span className="font-bold text-stone-800">{p.data.name}</span>
                          <span className={`text-[10px] block ${p.data.realm.color}`}>{p.data.realm.name}</span>
                        </div>
                      ))}
                    </div>
                    <div className="mt-4 text-xs text-stone-500 bg-stone-50 p-2 rounded">
                      {relationResult.target.lunar.year} {relationResult.target.lunar.month} {relationResult.target.lunar.day} {relationResult.target.lunar.time}
                    </div>
                  </div>

                  {/* Analysis Result */}
                  <div className="bg-white p-6 rounded-2xl shadow-md border border-stone-100 space-y-4">
                    <h3 className="font-bold text-rose-800 mb-4 flex items-center gap-2 border-b border-rose-100 pb-2">
                      <GitMerge size={18} /> 关系深度分析
                    </h3>

                    {/* 1. Zodiac Harm */}
                    <div className={`p-4 rounded-xl flex items-start gap-3 ${relationResult.analysis.isHarm ? 'bg-red-50 text-red-800' : 'bg-emerald-50 text-emerald-800'}`}>
                      {relationResult.analysis.isHarm ? <ShieldAlert size={20} className="shrink-0 mt-0.5" /> : <ShieldCheck size={20} className="shrink-0 mt-0.5" />}
                      <div>
                        <div className="font-bold text-sm">生肖根基判定</div>
                        <div className="text-sm mt-1 opacity-90">{relationResult.analysis.harmDesc}</div>
                      </div>
                    </div>

                    {/* 2. Complementary Stars */}
                    {relationResult.analysis.complements.length > 0 ? (
                      <div className="bg-amber-50 p-4 rounded-xl text-amber-900">
                        <div className="font-bold text-sm flex items-center gap-2 mb-2"><Sparkles size={16} /> 能量互补</div>
                        <ul className="list-disc list-inside text-sm space-y-1">
                          {relationResult.analysis.complements.map((c, i) => <li key={i}>{c}</li>)}
                        </ul>
                      </div>
                    ) : (
                      <div className="bg-stone-50 p-4 rounded-xl text-stone-500 text-sm text-center">
                        暂无显著的星宿互补，双方能量较为独立。
                      </div>
                    )}

                    {/* 3. Influence Type */}
                    <div className="bg-purple-50 p-4 rounded-xl text-purple-900">
                      <div className="font-bold text-sm mb-1">对方给您带来的主要影响：</div>
                      <div className="text-lg font-bold">{relationResult.analysis.influenceType}</div>
                      <div className="text-xs mt-2 opacity-70">
                        (影响指数: {relationResult.analysis.influenceScore > 0 ? '+' : ''}{relationResult.analysis.influenceScore} 分)
                      </div>
                    </div>
                  </div>
                </div>
              )}
            </div>
          ) : (
            // GUIDE MODE
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8 no-print">
              <div className="bg-white rounded-2xl shadow-xl border border-stone-200 overflow-hidden w-full mx-auto">
                <div className="bg-teal-50 px-8 py-4 border-b border-teal-100 flex items-center gap-2">
                  <CalendarCheck className="text-teal-600" size={20} />
                  <h2 className="font-bold text-teal-800 tracking-wide">出生择吉 · 预产期规划</h2>
                </div>
                <div className="p-8 md:p-10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      <label className="text-xs font-bold text-stone-400 uppercase tracking-widest">预产期 / 目标日期</label>
                      <input type="date" value={guideDueDate} onChange={(e) => setGuideDueDate(e.target.value)} className="w-full p-4 bg-stone-50 border border-stone-200 rounded-xl text-stone-800 outline-none focus:ring-2 focus:ring-teal-200 font-mono text-lg" />
                    </div>
                    <div className="space-y-3">
                      <label className="text-xs font-bold text-stone-400 uppercase tracking-widest">宝宝性别</label>
                      <div className="flex gap-4">
                        <button onClick={() => setGuideGender('male')} className={`flex-1 py-4 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${guideGender === 'male' ? 'bg-teal-800 text-white shadow-lg' : 'bg-stone-100 text-stone-400'}`}><Sun size={18} /> 男宝</button>
                        <button onClick={() => setGuideGender('female')} className={`flex-1 py-4 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${guideGender === 'female' ? 'bg-teal-800 text-white shadow-lg' : 'bg-stone-100 text-stone-400'}`}><RotateCcw size={18} /> 女宝</button>
                      </div>
                    </div>
                  </div>
                  <div className="mt-8 text-center">
                    <button onClick={calculateBirthGuide} className="px-12 py-4 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-full shadow-lg hover:shadow-teal-200/50 transform transition-all active:scale-95 text-lg tracking-widest flex items-center gap-2 mx-auto">
                      <Filter size={20} /> 筛选吉日吉时
                    </button>
                  </div>
                </div>
              </div>

              {guideLoading && <div className="text-center py-12 text-stone-400 animate-pulse">正在遍历天干地支，寻找上等命格...</div>}

              {guideResults.length > 0 && (
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {guideResults.map((res, idx) => (
                    <div key={idx} className="bg-white p-6 rounded-2xl shadow-md border border-stone-100 hover:shadow-xl transition-all relative overflow-hidden group">
                      <div className="absolute top-0 right-0 bg-teal-500 text-white text-xs font-bold px-3 py-1 rounded-bl-xl shadow-sm">得分: {res.score}</div>
                      <div className="flex items-center gap-2 mb-2 text-stone-400 text-xs font-mono">
                        <Calendar size={12} /> {res.dateStr} ({res.lunarStr})
                      </div>
                      <div className="flex flex-col gap-1 mb-4">
                        <div className="flex items-baseline gap-2">
                          <h4 className="text-2xl font-bold text-stone-800">{res.timeLabel}</h4>
                          <span className="text-xs text-teal-600 font-bold bg-teal-50 px-2 py-0.5 rounded">晚运: {res.keyStar}</span>
                        </div>
                        <div className="text-xs text-stone-500 font-mono bg-stone-50 p-1 rounded text-center">{res.timeRange}</div>
                      </div>
                      <div className="space-y-2 mb-4">
                        <div className="flex justify-between text-sm text-stone-600 border-b border-stone-50 pb-1"><span>年: {res.pillars[0]}</span><span>月: {res.pillars[1]}</span></div>
                        <div className="flex justify-between text-sm text-stone-600"><span>日: {res.pillars[2]}</span><span className="font-bold text-teal-700">时: {res.pillars[3]}</span></div>
                      </div>
                      <div className="flex gap-2 mt-4 flex-wrap">
                        {res.score >= 6 && <span className="text-[10px] bg-amber-100 text-amber-800 px-2 py-1 rounded flex items-center gap-1"><ThumbsUp size={10} /> 上等命格</span>}
                        {res.pillars.includes('天贵星') && <span className="text-[10px] bg-stone-100 text-stone-600 px-2 py-1 rounded">含天贵</span>}
                        {res.pillars.includes('天福星') && <span className="text-[10px] bg-stone-100 text-stone-600 px-2 py-1 rounded">含天福</span>}
                        {res.fanZhong && res.fanZhong.map((fz, i) => (
                          <span key={i} className="text-[10px] bg-purple-100 text-purple-800 px-2 py-1 rounded border border-purple-200">{fz}</span>
                        ))}
                      </div>

                      <div className="absolute bottom-4 right-4">
                        <button
                          onClick={() => handleGuideAi(res)}
                          className="text-[10px] bg-purple-100 hover:bg-purple-200 text-purple-700 px-3 py-1.5 rounded-full font-bold flex items-center gap-1 transition-colors shadow-sm"
                        >
                          <Bot size={12} /> 大师详解
                        </button>
                      </div>
                    </div>
                  ))}
                </div>
              )}

              {activeGuideAi && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-300">
                  <div className="bg-white w-full max-w-2xl max-h-[80vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col relative">
                    <div className="p-6 border-b border-stone-100 flex justify-between items-center bg-gradient-to-r from-purple-50 to-white">
                      <h3 className="text-xl font-bold text-purple-900 flex items-center gap-2"><Bot size={24} /> 命理大师深度解析</h3>
                      <button onClick={() => setActiveGuideAi(null)} className="p-2 hover:bg-stone-100 rounded-full transition-colors"><X size={20} className="text-stone-400" /></button>
                    </div>
                    <div className="p-8 overflow-y-auto custom-scrollbar flex-grow">
                      {activeGuideAi.loading ? (
                        <div className="flex flex-col items-center justify-center py-12 space-y-4 text-purple-600">
                          <div className="animate-spin"><RotateCcw size={32} /></div>
                          <p className="text-sm font-bold animate-pulse">大师正在凝神推演此日格局...</p>
                        </div>
                      ) : (
                        <div className="prose prose-purple max-w-none text-stone-700 leading-loose text-sm whitespace-pre-wrap">
                          {activeGuideAi.content}
                        </div>
                      )}
                    </div>
                    <div className="p-4 bg-stone-50 border-t border-stone-100 text-center text-xs text-stone-400">
                      AI生成内容仅供参考，命运掌握在自己手中
                    </div>
                  </div>
                </div>
              )}
            </div>
          )}

        </div>
      </div>

      {/* --- PRINT LAYOUT --- */}
      <div className="print-only font-serif text-black bg-white p-8 max-w-4xl mx-auto">
        {mode === 'analysis' && result && (
          <div>
            {/* Header */}
            <div className="text-center border-b-2 border-stone-800 pb-6 mb-8">
              <h1 className="text-4xl font-bold mb-3 tracking-widest text-stone-900">达摩一掌经 · 命理分析报告</h1>
              <div className="flex justify-center flex-wrap gap-8 text-sm font-medium text-stone-600">
                <span className="flex items-center gap-1"><Calendar size={14} /> 公历：{birthDate} {birthTime}</span>
                <span className="flex items-center gap-1"><Moon size={14} /> 农历：{result.lunar.year} {result.lunar.month} {result.lunar.day} {result.lunar.time}</span>
                <span className="flex items-center gap-1"><User size={14} /> 性别：{gender === 'male' ? '男' : '女'}</span>
              </div>
            </div>

            {/* Top Section: Chart & Summary */}
            <div className="flex flex-col md:flex-row gap-8 mb-8">
              {/* Left: Hand Chart */}
              <div className="w-full md:w-1/3 flex flex-col items-center">
                <HandGrid activePositions={result.pillars.map((p, i) => ({ pos: p.pos, label: ['年', '月', '日', '时'][i] }))} printMode={true} />

                <div className="w-full mt-4 space-y-2">
                  <div className="flex justify-between text-xs border-b border-stone-200 pb-1"><span>时分三刻</span><span className="font-bold">{result.advanced.timeSub.label}</span></div>
                  <div className="flex justify-between text-xs border-b border-stone-200 pb-1"><span>年月逢识</span><span className="font-bold">{result.advanced.shi.name}</span></div>
                  <div className="flex justify-between text-xs border-b border-stone-200 pb-1"><span>日遇吉凶</span><span className="font-bold">{result.advanced.dayStar.name}</span></div>
                </div>
              </div>

              {/* Right: Pillars & Fate */}
              <div className="w-full md:w-2/3 flex flex-col gap-6">
                {/* 4 Pillars Summary Grid */}
                <div className="grid grid-cols-4 gap-2">
                  {result.pillars.map((p, i) => (
                    <div key={i} className="border border-stone-800 rounded-lg p-3 text-center flex flex-col justify-between h-24">
                      <span className="text-xs font-bold text-stone-500 uppercase tracking-widest">{p.title}</span>
                      <span className="text-xl font-bold text-stone-900">{p.data.name}</span>
                      <span className="text-xs bg-stone-100 text-stone-600 px-1 py-0.5 rounded mx-auto">{p.data.realm.name}</span>
                    </div>
                  ))}
                </div>

                {/* Overall Fate */}
                <div className="flex-grow border-t border-b border-stone-200 py-4 flex flex-col justify-center">
                  <h3 className="font-bold text-lg mb-2 flex items-center gap-2"><Sparkles size={16} /> 命格总评</h3>
                  <p className="text-sm leading-7 text-justify text-stone-700">{result.analysis.overallFate}</p>
                  {result.analysis.overallAdvice && (
                    <div className="mt-4 bg-stone-50 p-3 rounded text-sm text-stone-600 border border-stone-200">
                      <strong className="text-stone-800">💡 重点提示：</strong> {result.analysis.overallAdvice}
                    </div>
                  )}
                </div>
              </div>
            </div>

            {/* Life Timeline */}
            <div className="break-inside-avoid mb-8 border border-stone-200 rounded-xl p-6">
              <h3 className="font-bold text-lg mb-6 border-l-4 border-stone-800 pl-3 flex items-center gap-2">人生流年轨迹</h3>
              <div className="grid grid-cols-2 gap-x-8 gap-y-6">
                {result.pillars.map((p, i) => (
                  <div key={i} className="break-inside-avoid relative pl-4 border-l-2 border-stone-300">
                    <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-stone-800 border-2 border-white"></div>
                    <div className="flex items-baseline justify-between mb-2">
                      <span className="font-bold text-base text-stone-800">{p.data.name} <span className="text-sm font-normal text-stone-500">({p.title})</span></span>
                      <span className="text-xs font-bold bg-stone-100 px-2 py-0.5 rounded">{p.age}</span>
                    </div>
                    <p className="text-xs text-stone-600 leading-relaxed text-justify">{p.specific_analysis}</p>
                  </div>
                ))}
              </div>
            </div>

            {/* Detailed Pillars Analysis */}
            <div className="break-inside-avoid mb-8">
              <h3 className="font-bold text-lg mb-4 border-l-4 border-stone-800 pl-3 flex items-center gap-2">四柱详解</h3>
              <div className="grid grid-cols-2 gap-6">
                {result.pillars.map((p, i) => (
                  <div key={i} className="break-inside-avoid border border-stone-300 rounded-lg p-4 bg-stone-50/50">
                    <div className="flex justify-between items-baseline mb-2 border-b border-stone-200 pb-1">
                      <h4 className="font-bold text-stone-900">{p.data.name} <span className="text-xs font-normal text-stone-500">({p.title})</span></h4>
                      <span className="text-xs font-bold bg-white border border-stone-200 px-2 rounded-full text-stone-600">{p.data.realm.trait}</span>
                    </div>
                    <p className="text-xs italic text-stone-500 font-serif mb-3 leading-relaxed">“{p.data.poem}”</p>
                    <div className="space-y-2">
                      <p className="text-xs text-stone-700"><strong><Scroll size={10} className="inline mr-1" />本义：</strong>{p.data.general_analysis}</p>
                      <p className="text-xs text-stone-700"><strong><MapPin size={10} className="inline mr-1" />落宫：</strong>{p.specific_analysis}</p>
                    </div>
                  </div>
                ))}
              </div>
            </div>

            {/* Advice & Special Findings */}
            <div className="grid grid-cols-2 gap-8 mb-8 break-inside-avoid">
              <div className="border border-stone-200 rounded-xl p-6">
                <h3 className="font-bold text-lg mb-4 flex items-center gap-2"><Heart size={16} /> 人生建议</h3>
                <div className="space-y-3 text-sm text-stone-700">
                  <p><strong className="text-stone-900">❤️ 择偶：</strong>{result.analysis.lifeAdvice.loveAdvice}</p>
                  <p><strong className="text-stone-900">🤝 交友：</strong>{result.analysis.lifeAdvice.friendAdvice}</p>
                  <p><strong className="text-stone-900">🌸 修养：</strong>{result.analysis.lifeAdvice.coreAdvice}</p>
                </div>
              </div>
              <div className="border border-stone-200 rounded-xl p-6">
                <h3 className="font-bold text-lg mb-4 flex items-center gap-2"><Star size={16} /> 特殊断语</h3>
                {(result.analysis.fanZhongFindings.length > 0 || result.analysis.interactionFindings.length > 0) ? (
                  <ul className="text-xs flex flex-col gap-2">
                    {result.analysis.fanZhongFindings.map((f, i) => (<li key={`fz-${i}`} className="bg-stone-50 p-1.5 rounded"><strong>[犯重] {f.star}:</strong> {f.desc}</li>))}
                    {result.analysis.interactionFindings.map((f, i) => (<li key={`in-${i}`} className="bg-stone-50 p-1.5 rounded"><strong>[{f.title}]</strong> {f.desc}</li>))}
                  </ul>
                ) : (
                  <p className="text-sm text-stone-500 italic">命盘格局平稳，无特殊冲克或吉格。</p>
                )}
              </div>
            </div>

            {/* AI Interpretation Section */}
            <div className="break-inside-avoid border-2 border-dashed border-stone-300 rounded-xl p-8 min-h-[300px]">
              <h3 className="font-bold text-lg mb-4 flex items-center gap-2 text-stone-800">
                <Bot size={20} /> AI 大师深度解盘
              </h3>
              {aiAnalysis ? (
                <div className="prose prose-sm max-w-none text-stone-700 leading-relaxed whitespace-pre-wrap columns-1">
                  {aiAnalysis}
                </div>
              ) : (
                <div className="h-full flex flex-col items-center justify-center text-stone-400 opacity-50 space-y-2">
                  <div className="text-4xl">✨</div>
                  <p className="text-sm font-medium uppercase tracking-widest center">此处预留大师批注</p>
                  <p className="text-xs">（请在屏幕上点击“AI大师深度解盘”生成内容后打印）</p>
                </div>
              )}
            </div>
          </div>
        )}

        {mode === 'relation' && relationResult && (
          <div>
            <div className="text-center border-b-2 border-stone-800 pb-6 mb-8">
              <h1 className="text-4xl font-bold mb-3 tracking-widest text-stone-900">达摩一掌经 · 人际合盘报告</h1>
              <div className="flex justify-center gap-8 text-sm font-medium text-stone-600">
                <span className="flex items-center gap-1"><User size={14} /> 主体：我 ({result.lunar.year})</span>
                <span className="flex items-center gap-1"><GitMerge size={14} /> 关系对象：{relName} ({relType === 'partner' ? '伴侣' : relType === 'colleague' ? '事业' : relType === 'friend' ? '朋友' : '家人'})</span>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-12 mb-8">
              {/* My Chart */}
              <div className="border border-stone-200 rounded-xl p-6">
                <h3 className="font-bold text-center border-b border-stone-100 pb-2 mb-4">我的命盘</h3>
                <div className="grid grid-cols-2 gap-2">
                  {result.pillars.map((p, i) => (
                    <div key={i} className="text-center p-2 bg-stone-50 rounded">
                      <div className="text-xs text-stone-400">{['年', '月', '日', '时'][i]}</div>
                      <div className="font-bold text-lg">{p.data.name}</div>
                      <div className="text-xs">{p.data.realm.name}</div>
                    </div>
                  ))}
                </div>
              </div>
              {/* Target Chart */}
              <div className="border border-stone-200 rounded-xl p-6">
                <h3 className="font-bold text-center border-b border-stone-100 pb-2 mb-4">{relName} ({relGender === 'male' ? '男' : '女'})</h3>
                <div className="grid grid-cols-2 gap-2">
                  {relationResult.target.pillars.map((p, i) => (
                    <div key={i} className="text-center p-2 bg-stone-50 rounded">
                      <div className="text-xs text-stone-400">{['年', '月', '日', '时'][i]}</div>
                      <div className="font-bold text-lg">{p.data.name}</div>
                      <div className="text-xs">{p.data.realm.name}</div>
                    </div>
                  ))}
                </div>
              </div>
            </div>

            <div className="space-y-6 break-inside-avoid">
              <div className={`p-6 rounded-xl border-l-4 ${relationResult.analysis.isHarm ? 'border-red-500 bg-red-50' : 'border-emerald-500 bg-emerald-50'}`}>
                <h3 className="font-bold text-lg mb-2">1. 生肖根基判定</h3>
                <p className="text-stone-800">{relationResult.analysis.isHarm ? '⚠️ ' : '✅ '}{relationResult.analysis.harmDesc}</p>
              </div>

              <div className="p-6 rounded-xl border border-stone-200 bg-white">
                <h3 className="font-bold text-lg mb-4">2. 能量互补分析</h3>
                {relationResult.analysis.complements.length > 0 ? (
                  <ul className="list-disc list-inside space-y-2 text-stone-700">
                    {relationResult.analysis.complements.map((c, i) => <li key={i}>{c}</li>)}
                  </ul>
                ) : <p className="text-stone-500 italic">双方命盘较为独立，无显著互补结构。</p>}
              </div>

              <div className="p-6 rounded-xl border border-stone-200 bg-purple-50">
                <h3 className="font-bold text-lg mb-2 text-purple-900">3. 深度影响评估</h3>
                <p className="text-xl font-bold text-purple-800 mb-2">{relationResult.analysis.influenceType}</p>
                <p className="text-sm text-purple-600">影响指数: {relationResult.analysis.influenceScore}</p>
              </div>
            </div>
          </div>
        )}

        {mode === 'guide' && (
          <div>
            <div className="text-center border-b-2 border-stone-800 pb-6 mb-8">
              <h1 className="text-4xl font-bold mb-3 tracking-widest text-stone-900">达摩一掌经 · 出生择吉报告</h1>
              <div className="text-sm font-medium text-stone-600">
                目标日期: {guideDueDate} · {guideGender === 'male' ? '男宝' : '女宝'}
              </div>
            </div>

            {activeGuideAi && activeGuideAi.item ? (
              // Specific Selection Detail
              <div className="space-y-8">
                <div className="bg-stone-50 border border-stone-200 p-6 rounded-xl text-center">
                  <h2 className="text-2xl font-bold text-stone-900 mb-2">{activeGuideAi.item.dateStr}</h2>
                  <p className="text-stone-600 font-mono mb-4">{activeGuideAi.item.lunarStr} · {activeGuideAi.item.timeLabel}</p>

                  <div className="grid grid-cols-4 gap-4 max-w-lg mx-auto">
                    <div className="p-3 bg-white rounded border border-stone-200">
                      <span className="block text-xs text-stone-400">年</span>
                      <span className="font-bold text-lg">{activeGuideAi.item.pillars[0]}</span>
                    </div>
                    <div className="p-3 bg-white rounded border border-stone-200">
                      <span className="block text-xs text-stone-400">月</span>
                      <span className="font-bold text-lg">{activeGuideAi.item.pillars[1]}</span>
                    </div>
                    <div className="p-3 bg-white rounded border border-stone-200">
                      <span className="block text-xs text-stone-400">日</span>
                      <span className="font-bold text-lg">{activeGuideAi.item.pillars[2]}</span>
                    </div>
                    <div className="p-3 bg-teal-50 rounded border border-teal-200">
                      <span className="block text-xs text-teal-600">时</span>
                      <span className="font-bold text-lg text-teal-900">{activeGuideAi.item.pillars[3]}</span>
                    </div>
                  </div>
                  <div className="mt-4 flex justify-center gap-2">
                    <span className="bg-stone-800 text-white px-3 py-1 rounded-full text-sm font-bold">综合得分: {activeGuideAi.item.score}</span>
                    {activeGuideAi.item.score >= 6 && <span className="bg-amber-100 text-amber-800 px-3 py-1 rounded-full text-sm font-bold">上等命格</span>}
                  </div>
                </div>

                <div className="border-t border-stone-200 pt-8">
                  <h3 className="font-bold text-xl mb-6 flex items-center gap-2"><Bot size={24} /> 大师深度解盘</h3>
                  <div className="prose prose-stone max-w-none leading-loose">
                    {activeGuideAi.content ? activeGuideAi.content : '（解析内容生成中...）'}
                  </div>
                </div>
              </div>
            ) : (
              // List View
              <div className="space-y-6">
                <p className="text-stone-500 italic text-center text-sm">（建议点击具体吉日的“大师详解”按钮后，打印该日期的深度报告）</p>
                <div className="grid grid-cols-2 gap-4">
                  {guideResults.slice(0, 12).map((res, i) => (
                    <div key={i} className="border border-stone-200 p-4 rounded-lg break-inside-avoid">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <div className="font-bold text-lg">{res.dateStr}</div>
                          <div className="text-xs text-stone-500">{res.lunarStr} · {res.timeLabel}</div>
                        </div>
                        <div className="font-bold text-xl text-stone-800">{res.score}分</div>
                      </div>
                      <div className="text-sm space-y-1 bg-stone-50 p-2 rounded">
                        <div className="flex justify-between"><span>年: {res.pillars[0]}</span><span>月: {res.pillars[1]}</span></div>
                        <div className="flex justify-between"><span>日: {res.pillars[2]}</span><span className="font-bold">时: {res.pillars[3]}</span></div>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            )}
          </div>
        )}

        <div className="text-center text-[10px] text-stone-400 mt-12 mb-4 border-t border-stone-200 pt-4">
          达摩一掌经排盘系统 · 仅供娱乐参考 · 命自我立
        </div>
      </div>
    </>
  );
};


export default OnePalmApp;
